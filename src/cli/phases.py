...
def phase_chunk(ctx: PipelineContext) -> PipelineContext:
    ...
    Uses smaller chunk size in demo mode. Shows chunk breakdown table.
    ...
    chunk_size = DEMO_CHUNK_SIZE if ctx.demo_mode else DEFAULT_CHUNK_SIZE
    ctx.chunk_size = chunk_size

    console.print(f"\n[bold]Chunking conversations...[/bold] (max {chunk_size:,} tokens)")
    chunks = chunk_conversations(ctx.conversations, max_tokens=chunk_size)
